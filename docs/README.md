...
| [bundled-fonts-cross-platform.md](bundled-fonts-cross-platform.md) | the manifest that keeps three font backends from disagreeing, and the four seams still open |
| [measuretext-textmetrics-fingerprinting.md](measuretext-textmetrics-fingerprinting.md) | ten-plus numbers from one call, why cumulative per-glyph noise backfired, and why the vertical fields needed a different fix |
