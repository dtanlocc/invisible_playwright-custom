...
def test_bind_keeps_adopting_after_the_first_success(monkeypatch):
    ...
    """
    guard = JobObjectGuard.__new__(JobObjectGuard)
    guard._adopted = set()
    adopted = []
    ...
    monkeypatch.setattr(_reaper, "find_processes", waves)
    bound = guard.bind(SessionToken.mint(), wait=6.0, settle=0.3)
...
def test_bind_does_not_wait_out_the_whole_deadline_once_the_tree_is_stable():
    ...
    import invisible_playwright._reaper as R
    original = R.find_processes
    R.find_processes = lambda _t: [_P(7)]
    try:
        started = time.monotonic()
        bound = guard.bind(SessionToken.mint(), wait=30.0, settle=0.3)
        elapsed = time.monotonic() - started
    finally:
        R.find_processes = original
